  ...
  <div class="page">
    ...
    <div id="hostPanel" style="display:none; width: 100%;">
      ...
      <nav class="tab-nav">
        ...
        <button class="tab-btn" data-tab="rooms">🏠 방 관리</button>
        ...
        <div class="card">
          <div class="room-code-box">
            ...
            <div style="margin-top:12px; width:100%;">
              <label style="font-size:13px; color:var(--text-muted);">호스트 관리 화면 링크</label>
              <div style="display:flex; gap:8px; margin-top:6px;">
                <input type="text" id="hostLinkInput" readonly style="flex:1; font-size:13px; padding:10px 12px; border-radius:10px; background:var(--surface-2); border:1px solid var(--border); color:var(--gold); cursor:pointer;" />
                <button class="btn btn-secondary" id="copyHostLinkBtn" style="width:auto; padding:10px 16px; font-size:13px;">복사</button>
              </div>
              <span style="font-size:12px; color:var(--text-muted); margin-top:4px; display:block;">⚠️ 이 링크는 호스트 전용입니다. 참가자에게 공유하지 마세요.</span>
            </div>
          </div>
        </div>
      ...
      <div class="tab-panel" id="tab-rooms">
        <div class="card">
          <div class="meta-row">
            <h2 style="margin:0;">생성된 방 목록</h2>
            <button class="btn btn-secondary" id="refreshRoomsBtn" style="width:auto;padding:8px 14px;font-size:13px;">🔄 새로고침</button>
          </div>
          <ul class="buzz-list" id="roomList" style="gap:10px;"></ul>
          <div class="empty-state" id="roomEmptyState">생성된 방이 없습니다.</div>
        </div>
      </div>
      ...
      <div class="tab-panel" id="tab-design">
        <div class="card">
          ...
          <div class="design-grid">
            ...
            <div class="design-item">
              <label>로고 텍스트 색상 (상단 브랜드)</label>
              <div class="color-row">
                <input type="color" id="colorLogoText" value="#f2f2f7" />
                <span id="colorLogoTextHex">#f2f2f7</span>
              </div>
            </div>
          </div>

          <!-- 카드 투명도 -->
          <div class="design-item" style="margin-top:14px;">
            <label>카드 투명도 <span id="cardOpacityVal">100%</span></label>
            <div style="display:flex; align-items:center; gap:12px; background:var(--surface-2); border:1px solid var(--border); border-radius:10px; padding:10px 14px;">
              <span style="font-size:12px; color:var(--text-muted);">투명</span>
              <input type="range" id="cardOpacity" min="10" max="100" value="100" step="5"
                style="flex:1; accent-color:var(--accent); cursor:pointer;" />
              <span style="font-size:12px; color:var(--text-muted);">불투명</span>
            </div>
            <span style="font-size:12px; color:var(--text-muted); margin-top:4px;">※ 정답 입력칸은 항상 불투명하게 유지됩니다.</span>
          </div>

          <!-- 배경 이미지 -->
          <div class="design-section-title">배경 이미지</div>
          <div class="card" style="background:rgba(255,200,87,0.07); border-color:#4a3e00; margin-bottom:0;">
            <p style="font-size:13px; color:var(--gold); margin:0 0 8px; font-weight:600;">⚡ 이미지 업로드 안내</p>
            <p style="font-size:13px; color:var(--text-muted); margin:0; line-height:1.6;">
              배경·로고 이미지는 <strong style="color:var(--text);">Cloudinary</strong>에 무료 업로드 후 URL을 붙여넣어 사용해요.<br />
              서버를 거치지 않아 500명+도 끊김 없이 빠르게 로딩돼요.<br />
              <a href="https://cloudinary.com/users/register_free" target="_blank" style="color:var(--accent);">→ Cloudinary 무료 가입</a>
              후 이미지를 업로드하고 "Copy URL"을 눌러 아래에 붙여넣으세요.
            </p>
          </div>

          <div class="design-item" style="margin-top:14px;">
            <label>PC 배경 이미지 URL
              <span style="font-size:11px; font-weight:400; color:var(--text-muted); margin-left:6px;">권장: 1920×1080px, 가로형</span>
            </label>
            <div style="display:flex; gap:8px; margin-top:4px;">
              <input type="text" id="bgUrlPc" placeholder="https://res.cloudinary.com/..." style="flex:1; font-size:13px;" />
              <button class="remove-btn" id="bgRemovePcBtn" style="white-space:nowrap;">제거</button>
            </div>
            <div class="bg-preview" id="bgPreviewPc" style="margin-top:8px;"><span>미리보기</span></div>
          </div>

          <div class="design-item" style="margin-top:14px;">
            <label>모바일 배경 이미지 URL
              <span style="font-size:11px; font-weight:400; color:var(--text-muted); margin-left:6px;">권장: 750×1334px, 세로형</span>
            </label>
            <div style="display:flex; gap:8px; margin-top:4px;">
              <input type="text" id="bgUrlMobile" placeholder="https://res.cloudinary.com/..." style="flex:1; font-size:13px;" />
              <button class="remove-btn" id="bgRemoveMobileBtn" style="white-space:nowrap;">제거</button>
            </div>
            <div class="bg-preview" id="bgPreviewMobile" style="margin-top:8px;"><span>미리보기</span></div>
          </div>
          ...
          <div class="design-item" style="margin-top:12px;">
            <label>로고 앞 아이콘 이미지 URL (선택)
              <span style="font-size:11px; font-weight:400; color:var(--text-muted); margin-left:6px;">권장: 64×64px, PNG 투명배경</span>
            </label>
            <div style="display:flex; gap:8px; margin-top:4px; align-items:center;">
              <input type="text" id="iconUrl" placeholder="https://res.cloudinary.com/..." style="flex:1; font-size:13px;" />
              <button class="remove-btn" id="iconRemoveBtn" style="white-space:nowrap;">제거</button>
            </div>
            <div id="iconPreview" style="margin-top:8px; display:flex; align-items:center; gap:8px; min-height:32px;">
              <span style="font-size:13px; color:var(--text-muted);">URL 입력 후 저장하면 브랜드 앞에 표시돼요</span>
            </div>
          </div>
          <div class="design-item" style="margin-top:12px;">
            <label>로고 이미지 URL (텍스트 대신 사용, 선택)
              <span style="font-size:11px; font-weight:400; color:var(--text-muted); margin-left:6px;">권장: 200×60px, PNG 투명배경</span>
            </label>
            <div style="display:flex; gap:8px; margin-top:4px; align-items:center;">
              <input type="text" id="logoUrl" placeholder="https://res.cloudinary.com/..." style="flex:1; font-size:13px;" />
              <button class="remove-btn" id="logoRemoveBtn" style="white-space:nowrap;">제거</button>
            </div>
            <div class="img-upload-box" id="logoPreview" style="margin-top:8px; border-style:solid; pointer-events:none;">
              <span>로고 URL 입력 후 미리보기</span>
            </div>
          </div>
